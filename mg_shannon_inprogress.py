...
class MinorityGameWithStrategyTable(MinorityGame):
    ...
    def run_game(self):
        ...
        for j in np.arange(0.0,1.1,0.1):
            ...
            for h in np.arange(0.0,1.1,0.1):
                ...
                for i in range(self.run_num):
                    ...
                    sums = sum(strategy_freq)
                    for i in range(len(strategy_freq)):
                        if strategy_freq[i] != 0:
                            shan -= (strategy_freq[i]/sums) * math.log(strategy_freq[i]/sums)
                    #shann = shan * len(self.agent_pool)
                    shannon.append(shan)
        ...
        print(strategy_freq)
        return mean_list,stdd_list,avg_win,volatility,avolatility,shannon
...
if __name__ == "__main__":
    ...
    # print(m.winner_for_diff_group())
        mean_list,stdd_list,avg_win,volatility,avolatility,shannon = m.run_game()
        for i in range(1,len(shannon)):
            shann=shannon[i]+shannon[i-1]
    print(shann)
